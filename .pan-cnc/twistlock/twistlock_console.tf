...
resource "kubernetes_persistent_volume_claim" "twistlock_console" {
  ...
  spec {
    ...
    storage_class_name = "twistlock-console-storage-class"
  }

  depends_on = ["kubernetes_storage_class.twistlock-console-storage-class",
                "kubernetes_namespace.twistlock"]
}
...
resource "kubernetes_deployment" "twistlock_console" {
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name  = "twistlock-console"
          image = "registry-auth.twistlock.com/tw_${var.access_token}/twistlock/console:console_19_11_480"
  ...
  provisioner "local-exec" {
    command = "./check_health.sh ${kubernetes_service.twistlock_console.load_balancer_ingress.0.ip}"
  }
  
  depends_on = ["kubernetes_service_account.twistlock_console",
                "kubernetes_service.twistlock_console",
                "kubernetes_namespace.twistlock",
                "kubernetes_config_map.twistlock_console",
                "kubernetes_persistent_volume_claim.twistlock_console" ]
}